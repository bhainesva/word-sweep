...
function App() {
  ...
  const [easyMode, setEasyMode] = useState(Boolean(localStorage.getItem(STORAGE_KEY)) || false);
  ...
  return (
    ...
    <header className="flex justify-between items-center">
      <div></div>
      <h1>Word Sweep!</h1>  
      <button onClick={() => {setSettingsOpen(old => !old)}}><FaGear /></button>
    </header>
...
function Settings(props: {onClose: () => void, checked: boolean, onChange: (checked: boolean) => void}) {
  return (
    <div className="modal flex items-center">
      <button className="absolute top-10 right-10" onClick={props.onClose}><IoClose /></button>
      <label className="flex justify-center gap-4">
        <input onChange={val => props.onChange(val.target.checked)} type="checkbox" id="easymode" checked={props.checked}/>  
        <div className="text-lg">
          If this checkbox is checked, inputs will highlight green as soon as the correct word is typed in them. If it is unchecked (the default), the inputs will only become green once all 3 words are correct.
        </div>
      </label>
    </div>
  )
...
function Questions(props: {easyMode: boolean}) {
  ...
  const allCorrect = inputs.every((input, i) => input === selectedCard.words[i].word);
  ...
  return (
    <>
      ...
      <div className="flex gap-4">
        <input value={value} ref={ref} className={cls} onChange={e => onChange(e.target.value)}></input>  
        <button className="bold small" onClick={() => {props.onChange(word.word)}}>Reveal</button>
      </div>
    </>
  )
})
